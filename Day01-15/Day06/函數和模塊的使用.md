...
> **說明：**Python的math模塊中其實已經有一個factorial函數了，事實上要計算階乘可以直接使用這個現成的函數而不用自己定義。下面例子中的某些函數其實Python中也是內置了，我們這裏是爲了講解函數的定義和使用才把它們又實現了一遍，實際開發中不建議做這種低級的重複性的工作。
...
但是如果將代碼寫成了下面的樣子，那麽程序中調用的是最後導入的那個`foo`，因爲後導入的foo覆蓋了之前導入的`foo`。
